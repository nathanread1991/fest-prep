...
def setup_database_config():
    ...
    db_password = get_input("Database password", "festival_pass", required=False, password=True)
...
def setup_api_keys():
    ...
    if api_keys['CLASHFINDER_USERNAME']:
        ...
        api_keys['CLASHFINDER_PRIVATE_KEY'] = get_input("Clashfinder private key", required=False, password=True)
    else:
    ...
    api_keys['SPOTIFY_CLIENT_SECRET'] = get_input("Spotify Client Secret", required=False, password=True)
...
def setup_oauth_providers():
    ...
    oauth_config['GOOGLE_CLIENT_ID'] = get_input("Google OAuth Client ID", required=False)
    if oauth_config['GOOGLE_CLIENT_ID']:
        print("🔐 Google OAuth Client Secret (will be hidden as you type):")
        oauth_config['GOOGLE_CLIENT_SECRET'] = get_input("Google OAuth Client Secret", required=False, password=True)
    else:
    ...
    oauth_config['APPLE_CLIENT_ID'] = get_input("Apple Client ID (Services ID)", required=False)
    if oauth_config['APPLE_CLIENT_ID']:
        print("🔐 Apple Client Secret (will be hidden as you type):")
        oauth_config['APPLE_CLIENT_SECRET'] = get_input("Apple Client Secret", required=False, password=True)
        print("🔐 Apple Private Key (will be hidden as you type):")
        oauth_config['APPLE_PRIVATE_KEY'] = get_input("Apple Private Key (full content)", required=False, password=True)
    ...
    if use_same_google:
        ...
        oauth_config['YOUTUBE_OAUTH_CLIENT_SECRET'] = oauth_config['GOOGLE_CLIENT_SECRET']
    else:
        oauth_config['YOUTUBE_OAUTH_CLIENT_ID'] = get_input("YouTube OAuth Client ID", required=False)
        if oauth_config['YOUTUBE_OAUTH_CLIENT_ID']:
            print("🔐 YouTube OAuth Client Secret (will be hidden as you type):")
            oauth_config['YOUTUBE_OAUTH_CLIENT_SECRET'] = get_input("YouTube OAuth Client Secret", required=False, password=True)
        else:
    ...
    if oauth_config['X_CLIENT_ID']:
        ...
        oauth_config['X_CLIENT_SECRET'] = get_input("X (Twitter) Client Secret", required=False, password=True)
    else:
...
def setup_additional_config():
    ...
    config['SESSION_EXPIRE_HOURS'] = get_input("Session expiry hours", "24", required=False)
    ...
    if setup_vapid:
        ...
        if config['VAPID_PUBLIC_KEY']:
            ...
            config['VAPID_PRIVATE_KEY'] = get_input("VAPID Private Key", required=False, password=True)
            config['VAPID_EMAIL'] = get_input("VAPID Email (contact email)", required=False)
        else:
...
def setup_admin_config():
    ...
    admin_password = get_input("Admin password", "admin123", required=False, password=True)
...
"""
    ...
    env_path = Path('.env')
    if env_path.exists():
        backup_existing = get_yes_no("⚠️  .env file already exists. Create backup?", True)
        if backup_existing:
...
def print_next_steps():
    ...
    print("   - Or manually: python -m uvicorn festival_playlist_generator.main:app --reload")
    print()
    print("7. 🧪 Run tests:")
    ...
    print("   - Make sure redirect URIs are set to: http://localhost:8000/auth/callback")
    print()
