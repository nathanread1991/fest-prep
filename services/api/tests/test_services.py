...
class TestFestivalService:
    ...
    async def test_create_festival_with_artist_validation(
        ...
        mock_cache, sample_festival, sample_artist
    ):
...
class TestPlaylistService:
    ...
    async def playlist_service(
        self, mock_playlist_repo, mock_festival_repo, 
        mock_cache, mock_spotify_service
    ):
    ...
    async def test_create_playlist_with_festival_validation(
        self, playlist_service, mock_playlist_repo, mock_festival_repo,
        mock_cache, sample_playlist
    ):
        ...
        # Assert
        assert result == sample_playlist
        mock_festival_repo.get_by_id.assert_called_once_with(sample_playlist.festival_id)
    ...
    async def test_sync_to_spotify_success(
        self, playlist_service, mock_playlist_repo, mock_spotify_service, sample_playlist
    ):
...
class TestSpotifyService:
    ...
    async def test_search_artist_success(self, spotify_service):
        ...
        with patch.object(spotify_service.client, 'get', return_value=mock_response) as mock_get:
    ...
    async def test_create_playlist_success(self, spotify_service):
        ...
        with patch.object(spotify_service.client, 'get', return_value=user_response):
            with patch.object(spotify_service.client, 'post', return_value=playlist_response):
                # Act
    ...
    async def test_add_tracks_to_playlist_batching(self, spotify_service):
        ...
        with patch.object(spotify_service.client, 'post', return_value=mock_response) as mock_post:
            # Act
...
class TestSetlistFmService:
    ...
    async def test_get_artist_setlists_success(self, setlistfm_service):
        ...
        mock_response.json.return_value = {
            "setlist": [
                {"id": "setlist1", "artist": {"name": "Test Artist"}}
            ]
        }
        mock_response.raise_for_status = MagicMock()
    ...
    async def test_retry_with_exponential_backoff(self, setlistfm_service):
        ...
        with patch.object(setlistfm_service.client, 'get', side_effect=failing_then_success):
            # Act
...
class TestArtistServiceAdditional:
    ...
    async def test_get_artist_by_name(self, artist_service, mock_cache, mock_artist_repo):
    ...
    async def test_get_artist_by_spotify_id(self, artist_service, mock_cache, mock_artist_repo):
    ...
    async def test_bulk_delete_artists(self, artist_service, mock_cache, mock_artist_repo):
    ...
    async def test_get_orphaned_artist_count(self, artist_service, mock_cache, mock_artist_repo):
...
class TestFestivalServiceAdditional:
    ...
    async def test_get_festival_by_name(self, festival_service, mock_cache, mock_festival_repo):
    ...
    async def test_get_upcoming_festivals(self, festival_service, mock_cache, mock_festival_repo):
    ...
    async def test_update_festival(self, festival_service, mock_cache, mock_festival_repo):
    ...
    async def test_get_festival_count(self, festival_service, mock_cache, mock_festival_repo):
...
class TestCacheServiceComplete:
    ...
    async def test_get_client_creates_pool(self):
        ...
        with patch('festival_playlist_generator.services.cache_service.redis.ConnectionPool') as mock_pool_class:
            mock_pool = AsyncMock()
            mock_pool_class.from_url.return_value = mock_pool
            
            with patch('festival_playlist_generator.services.cache_service.redis.Redis') as mock_redis_class:
...
class TestArtistServiceComplete:
    ...
    async def test_get_artist_by_id_with_relationships(
        ...
        # Act
        result = await artist_service.get_artist_by_id(artist.id, load_relationships=True)
...
class TestFestivalServiceComplete:
    ...
    async def test_get_festival_by_id_with_relationships(
        ...
        # Act
        result = await festival_service.get_festival_by_id(festival.id, load_relationships=True)
    ...
    async def test_get_upcoming_festivals_with_relationships(
        ...
        # Act
        result = await festival_service.get_upcoming_festivals(limit=5, load_relationships=True)
    ...
    async def test_search_festivals_cache_hit(
        ...
        # Arrange
        sample_festival = Festival(id=uuid4(), name="Test Festival", dates=[datetime.utcnow()], location="Test City")
        mock_festival_repo.search_paginated.return_value = ([sample_festival], 1)
...
class TestPlaylistServiceComplete:
    ...
    async def test_get_playlist_by_id_with_relationships(
        ...
        # Act
        result = await playlist_service.get_playlist_by_id(playlist.id, load_relationships=True)
