...
class TestFestivalEndpoints:
    ...
    async def test_update_festival(self, test_client, test_festival):
        ...
        response = await test_client.put(
            f"/api/v1/festivals/{test_festival.id}",
            json=update_data
        )
    ...
    async def test_delete_festival(self, test_client, test_festival, async_session):
        ...
        stmt = delete(festival_artists).where(festival_artists.c.festival_id == test_festival.id)
...
class TestCleanArchitecture:
    ...
    async def test_controllers_use_services(self):
        ...
        assert hasattr(festivals, 'get_festival_service') or 'FestivalService' in dir(festivals)
        
        # Check playlist endpoints
        from festival_playlist_generator.api.endpoints import playlists
        assert hasattr(playlists, 'get_playlist_service') or 'PlaylistService' in dir(playlists)
    ...
    async def test_no_direct_db_access_in_controllers(self):
        ...
        import inspect
        from festival_playlist_generator.api.endpoints import artists, festivals, playlists, users
        ...
        for module in modules_to_check:
            ...
            assert 'Depends(get_' in source or 'Service' in source, \
