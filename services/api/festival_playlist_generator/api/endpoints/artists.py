...
from festival_playlist_generator.schemas.artist import (
    Artist,
    ArtistCreate,
    ArtistUpdate,
)
...
async def create_artist(
    ...
    return formatter.created_response(
        data=artist_data.model_dump(), message="Artist created successfully"
    )
...
async def get_artist(
    ...
    return formatter.success_response(
        data=artist_data.model_dump(), message="Artist retrieved successfully"
    )
...
async def update_artist(
    ...
    return formatter.success_response(
        data=artist_data.model_dump(), message="Artist updated successfully"
    )
...
async def search_artists(
    ...
    # Search via service
    artists, total_count = await artist_service.search_artists(
        search=q, page=page, per_page=per_page, order_by="created_at", order_desc=True
    )
    ...
    festival_data = [
        {
            ...
            "name": festival.name,
            "dates": (
                [date.isoformat() for date in festival.dates] if festival.dates else []
            ),
