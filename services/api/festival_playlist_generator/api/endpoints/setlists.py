...
from festival_playlist_generator.schemas.setlist import Setlist, SetlistCreate, SetlistUpdate
...
async def get_artist_setlists(
    ...
    fetch_external: bool = Query(False, description="Fetch from external API if no local data"),
    ...
    query = select(SetlistModel).filter(SetlistModel.artist_id == artist_id).order_by(SetlistModel.date.desc())
    ...
    if not setlists and fetch_external:
        try:
            ...
            logger.error(f"Error fetching external setlists for artist {artist.name}: {e}")
...
async def get_recent_artist_setlists(
    request: Request,
    artist_id: UUID,
    limit: int = Query(5, ge=1, le=20, description="Number of recent setlists to return"),
    fetch_external: bool = Query(True, description="Fetch from external API if no local data")
):
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            query = select(SetlistModel).filter(SetlistModel.artist_id == artist_id).order_by(SetlistModel.date.desc()).limit(limit)
            result = await db.execute(query)
            setlists = result.scalars().all()
            ...
            for setlist in setlists:
                setlist_data.append({
                    ...
                    "songs": setlist.songs[:10],  # Limit to first 10 songs for preview
                    "tour_name": setlist.tour_name,
                    "festival_name": setlist.festival_name,
                    "source": setlist.source
                })
...
async def get_setlist(
    ...
    query = select(SetlistModel).options(selectinload(SetlistModel.artist)).filter(SetlistModel.id == setlist_id)
    result = await db.execute(query)
...
async def list_setlists(
    ...
    # Apply filters
    if artist_name:
        query = query.join(SetlistModel.artist).filter(ArtistModel.name.ilike(f"%{artist_name}%"))
    ...
    return formatter.success_response(
        data=setlist_data,
        message=f"Found {len(setlist_data)} setlists"
    )