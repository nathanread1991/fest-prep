...
from festival_playlist_generator.schemas.festival import (
    Festival,
    FestivalCreate,
    FestivalUpdate,
)
from festival_playlist_generator.services.festival_enrichment import (
    festival_enrichment_service,
)
...
async def enrich_festival_data(clashfinder_id: str) -> JSONResponse:
    ...
    """
    try:
        enriched_data = await festival_enrichment_service.fetch_from_clashfinder(
            clashfinder_id
        )
...
async def scrape_festival_by_name(
    ...
    try:
        ...
        if url:
            ...
            festival_data = await festival_scraper.search_and_scrape_festival(
                name, year
            )
        ...
        for festival_data in scraped_festivals:
            try:
                ...
                if existing_festival:
                    ...
                    if festival_data["artists"] and len(festival_data["artists"]) > len(
                        existing_festival.artists
                    ):
                        # Add new artists
                    ...
                    # Parse dates
                    dates = [
                        datetime.strptime(d, "%Y-%m-%d") for d in festival_data["dates"]
                    ]
                    ...
                    # Add artists
                    for artist_name in festival_data.get("artists", [])[
                        :50
                    ]:  # Limit to 50 artists
                        artist_result = await db.execute(
...
async def create_festival(
    ...
    try:
        ...
        await db.execute(
            select(FestivalModel)
            .options(selectinload(FestivalModel.artists))
            .filter(FestivalModel.id == db_festival.id)
        )
        ...
        return JSONResponse(
            content={
                ...
                "data": {
                    ...
                    "branding_extracted_at": (
                        db_festival.branding_extracted_at.isoformat()
                        if db_festival.branding_extracted_at
                        else None
                    ),
                },
            ...
            festival_data.append(
                {
                    ...
                    "name": festival.name,
                    "dates": (
                        [date.isoformat() for date in festival.dates]
                        if festival.dates
                        else []
                    ),
                    "location": festival.location,
                    ...
                    "accent_colors": festival.accent_colors,
                    "branding_extracted_at": (
                        festival.branding_extracted_at.isoformat()
                        if festival.branding_extracted_at
                        else None
                    ),
                }
            )
...
async def get_festival(
    ...
    festival = await festival_service.get_festival_by_id(
        festival_id, load_relationships=True
    )
    ...
    return Festival(
        ...
        ticket_url=festival.ticket_url,
        artists=(
            [artist.name for artist in festival.artists]
            if hasattr(festival, "artists")
            else []
        ),
...
async def update_festival(
    ...
    # Update via service
    updated_festival = await festival_service.update_festival(
        festival, artist_ids=artist_ids
    )
...
async def search_festivals(
    ...
    # Search via service
    festivals, total = await festival_service.search_festivals(
        search=q, page=page, per_page=per_page, order_by="dates", order_desc=False
    )
