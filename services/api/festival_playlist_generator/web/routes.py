...
async def debug_auth(request: Request):
    ...
    return templates.TemplateResponse("debug_auth.html", {
        "request": request
    })
...
async def artists_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    query = select(
        ArtistModel,
        func.count(SetlistModel.id).label('setlist_count')
    ).outerjoin(SetlistModel).group_by(ArtistModel.id).order_by(ArtistModel.name)
    ...
    response = templates.TemplateResponse("artists.html", {
        "request": request, 
        "artists": artists_with_counts
    })
...
async def festivals_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    response = templates.TemplateResponse("festivals.html", {
        "request": request,
        "festivals": festivals
    })
...
async def festival_detail(
    ...
    from festival_playlist_generator.core.caching import cache_manager, HTTPCacheManager
    from festival_playlist_generator.services.image_url_helper import convert_to_proxy_url
    
    # Check if we should bypass cache (for refresh or admin updates)
    bypass_cache = request.query_params.get('_refresh') or request.query_params.get('_nocache')
    ...
    sorted_artists = sorted(
        festival.artists,
        key=lambda a: (a.spotify_followers or 0),
        reverse=True
    )
...
async def playlists_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    response = templates.TemplateResponse("playlists.html", {
        "request": request,
        "playlists": playlists,
        "user": user
    })
...
async def artist_detail(
    ...
    from festival_playlist_generator.services.image_url_helper import convert_to_proxy_url, convert_track_images
    ...
    try:
        ...
        recent_setlists_query = select(SetlistModel).filter(
            SetlistModel.artist_id == UUID(artist_id)
        ).order_by(SetlistModel.date.desc()).limit(10)
        ...
        if not recent_setlists:
            try:
                ...
                if settings.SETLIST_FM_API_KEY:
                    ...
                    external_setlists = await analyzer.get_artist_setlists(artist.name, limit=10)
        ...
        if not top_songs and artist.spotify_id:
            try:
                ...
                logger.error(f"Error fetching Spotify top tracks for {artist.name}: {e}")
...
async def search_page(
    ...
    return templates.TemplateResponse(
        "search.html", 
        {"request": request, "query": q, "search_type": type}
    )
