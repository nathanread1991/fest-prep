...
async def initiate_oauth(provider: str):
    ...
    try:
        ...
        return RedirectResponse(
            url=oauth_data["authorization_url"],
            status_code=302
        )
...
async def oauth_callback(
    ...
    if error:
        ...
        return RedirectResponse(
            url="/auth/login?error=oauth_denied",
            status_code=302
        )
    ...
    if not code or not state:
        ...
        return RedirectResponse(
            url="/auth/login?error=invalid_callback",
            status_code=302
        )
...
async def account_settings_page(
    ...
    return templates.TemplateResponse("auth/account_settings.html", {
        "request": request,
        "user": user
    })
...
async def privacy_consent_page(
    ...
    return templates.TemplateResponse("auth/privacy_consent.html", {
        "request": request,
        "user_id": user_id,
...
async def privacy_preferences_page(
    ...
    return templates.TemplateResponse("auth/privacy_preferences.html", {
        "request": request,
        "user": user
    })
...
async def export_user_data_page(request: Request, user: UserSchema = Depends(require_auth)):
    """Display data export page."""
    return templates.TemplateResponse("auth/export_data.html", {
        "request": request,
        "user": user,
...
async def delete_account_page(request: Request, user: UserSchema = Depends(require_auth)):
    """Display account deletion confirmation page."""
    return templates.TemplateResponse("auth/delete_account.html", {
        "request": request,
        "user": user,
