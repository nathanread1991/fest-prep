...
async def admin_festivals(
    ...
    try:
        ...
        response = templates.TemplateResponse("admin/festivals.html", {
            "request": request,
            "admin_user": admin_user,
            "festivals": festivals
        })
...
async def admin_new_festival(
    ...
    """New festival form."""
    return templates.TemplateResponse("admin/festival_form.html", {
        "request": request,
        "admin_user": admin_user,
        "festival": None,
        "action": "Create"
...
async def admin_edit_festival(
    ...
    try:
        ...
        if not festival:
            return templates.TemplateResponse("admin/festivals.html", {
                "request": request,
                "admin_user": admin_user,
                "festivals": [],
        ...
        dates_str = ", ".join([d.strftime('%Y-%m-%d') for d in festival.dates]) if festival.dates else ""
        genres_str = ", ".join(festival.genres) if festival.genres else ""
        artists_str = ", ".join([a.name for a in festival.artists]) if festival.artists else ""
        accent_colors_str = ", ".join(festival.accent_colors) if festival.accent_colors else ""
...
async def admin_create_festival(
    ...
    try:
        ...
        accent_color_list = []
        if accent_colors:
            accent_color_list = [c.strip() for c in accent_colors.split(',') if c.strip()]
...
async def admin_artists(
    ...
    try:
        ...
        response = templates.TemplateResponse("admin/artists.html", {
            "request": request,
            "admin_user": admin_user,
            "artists": artists,
            "page": page,
...
async def admin_new_artist(
    ...
    """New artist form."""
    return templates.TemplateResponse("admin/artist_form.html", {
        "request": request,
        "admin_user": admin_user,
        "artist": None,
        "action": "Create"
...
async def admin_spotify_search(
    ...
    try:
        ...
        if artists:
            ...
            return {"success": False, "message": f"No Spotify results found for '{artist_name}'"}
...
async def admin_create_artist(
    ...
    try:
        ...
        if spotify_id:
            existing_spotify_query = select(ArtistModel).filter(ArtistModel.spotify_id == spotify_id)
        ...
        if musicbrainz_id:
            existing_mb_query = select(ArtistModel).filter(ArtistModel.musicbrainz_id == musicbrainz_id)
...
async def admin_delete_festival(
    ...
    try:
        ...
        delete_relationships = delete(festival_artists).where(festival_artists.c.festival_id == festival_uuid)
        await db.execute(delete_relationships)
        logger.info(f"Deleted festival_artists relationships for festival {festival_id}")
        ...
        logger.info(f"Successfully deleted festival {festival_id}, rows affected: {result.rowcount}")
        ...
        await db.rollback()
        return RedirectResponse(url="/admin/festivals?error=delete_failed", status_code=303)
...
async def admin_update_festival(
    ...
    try:
        ...
        # Parse accent colors
        accent_color_list = [c.strip() for c in accent_colors.split(',') if c.strip()] if accent_colors else []
...
async def admin_bulk_delete_festivals(
    ...
    try:
        ...
        await db.execute(delete_relationships)
        logger.info(f"Deleted festival_artists relationships for {len(festival_ids)} festivals")
...
async def admin_edit_artist(
    ...
    try:
        ...
        if not artist:
            return templates.TemplateResponse("admin/artists.html", {
                "request": request,
                "admin_user": admin_user,
                "artists": [],
        ...
        return templates.TemplateResponse("admin/artist_edit_form.html", {
            "request": request,
            "admin_user": admin_user,
            "artist": artist,
            "action": "Update",
...
async def admin_search_artists(
    ...
    try:
        ...
        if source == "spotify":
            ...
            try:
                spotify_artists = spotify_artist_service.search_artists_multiple(search_term, limit=10)
                ...
                return JSONResponse(
                    status_code=500,
                    content={"success": False, "message": f"Spotify search error: {str(e)}"}
                )
        
        else:
            # Search local database using advanced fuzzy matching
            from festival_playlist_generator.services.advanced_fuzzy_matcher import advanced_fuzzy_search
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code != 200:
                return {"success": False, "message": f"Setlist.fm API error: {response.status_code}"}
            ...
            if artists:
                ...
                return {"success": False, "message": f"No Setlist.fm results found for '{artist_name}'"}
    ...
    try:
        ...
        if existing_artist and existing_artist.id != artist_uuid:
            if not force_override:
                ...
                logger.info(f"Unlinking Spotify ID {spotify_id} from artist '{existing_artist.name}' (ID: {existing_artist.id})")
        ...
        return JSONResponse(
            ...
            content={
                "success": True,
                "message": "Spotify data applied successfully" + (" (unlinked from other artist)" if force_override and existing_artist else ""),
                "artist": {
                    "id": str(artist.id),
                    "name": artist.name,
                    "spotify_id": artist.spotify_id,
...
async def admin_apply_setlistfm_data(
    ...
    try:
        ...
        # Fetch setlists
        setlist_result = await setlist_enrichment_service.enrich_artists([str(artist_uuid)], db, limit=10)
...
async def admin_enrich_artist(
    ...
    try:
        ...
        try:
            spotify_result = await artist_enrichment_service.enrich_artists([str(artist_uuid)], db)
        ...
        try:
            setlist_result = await setlist_enrichment_service.enrich_artists([str(artist_uuid)], db)
...
async def admin_delete_artist(
    ...
    try:
        ...
        if not artist:
            ...
            return RedirectResponse(url="/admin/artists?error=not_found", status_code=303)
        ...
        if artist.festivals:
            ...
            logger.warning(f"Cannot delete artist {artist.name} - still associated with festivals: {festival_names}")
            return RedirectResponse(
                url=f"/admin/artists?error=has_festivals&artist={artist.name}&festivals={len(artist.festivals)}", 
                status_code=303
            )
        
        logger.info(f"Deleting artist {artist.name} (ID: {artist_id}) - Orphaned: {not artist.spotify_id and len(artist.setlists) == 0}")
        
        # First, delete relationships in festival_artists table (should be empty at this point)
        from festival_playlist_generator.models.festival import festival_artists
        delete_relationships = delete(festival_artists).where(festival_artists.c.artist_id == artist_uuid)
        await db.execute(delete_relationships)
        ...
        await db.rollback()
        return RedirectResponse(url="/admin/artists?error=delete_failed", status_code=303)
        await db.execute(delete_setlists)
    ...
    try:
        ...
        for artist_id in artist_ids:
            try:
                ...
                return {"success": False, "message": f"Invalid artist ID format: {artist_id}"}
        ...
        from festival_playlist_generator.models.festival import festival_artists
        delete_relationships = delete(festival_artists).where(festival_artists.c.artist_id.in_(artist_uuids))
        await db.execute(delete_relationships)
...
async def admin_enrich_all_artists(
    ...
    try:
        ...
        # Enrich artists
        enrichment_result = await artist_enrichment_service.enrich_artists(artist_ids, db)
...
async def admin_delete_user(
    ...
    try:
        ...
        user_result = await db.execute(select(UserModel).where(UserModel.id == user_uuid))
        ...
        delete_preferences = delete(UserSongPreference).where(UserSongPreference.user_id == user_uuid)
        ...
        logger.info(f"User {user_email} deleted - active sessions will expire naturally within {24} hours")
        ...
        await db.rollback()
        return RedirectResponse(url="/admin/users?error=delete_failed&_refresh=1", status_code=303)
...
async def admin_refresh_festival_branding(
    ...
    try:
        ...
        from festival_playlist_generator.services.festival_scraper import FestivalScraper
        import httpx
        
        festival_uuid = UUID(festival_id)
        
        # Get the festival
        result = await db.execute(select(FestivalModel).where(FestivalModel.id == festival_uuid))
        festival = result.scalar_one_or_none()
        ...
        if not festival_url:
            ...
            logger.info(f"No URL found for festival {festival.name}, attempting to search...")
            return JSONResponse(
                status_code=400,
                content={
                    "success": False,
        ...
        try:
            branding = await brand_extractor.extract_festival_branding(html_content, festival_url)
...
async def admin_duplicates(
    ...
    try:
        ...
        try:
            ...
            response = templates.TemplateResponse("admin/duplicates.html", {
                "request": request,
                "admin_user": admin_user,
...
async def admin_preview_merge(
    ...
    try:
        ...
        if not primary_id or not secondary_ids:
            return JSONResponse(
                status_code=400,
                content={"success": False, "message": "Missing primary_id or secondary_ids"}
            )
...
async def admin_merge_artists(
    ...
    try:
        ...
        logger.info(f"Admin {admin_user} merging artists: primary={primary_id}, secondaries={secondary_ids}")
        ...
        try:
            ...
            if result.success:
                logger.info(f"Successfully merged artists: {result.merged_artist_names} into {result.primary_artist_name}")
                return JSONResponse(
                    status_code=200,
                    content={
                        "success": True,
