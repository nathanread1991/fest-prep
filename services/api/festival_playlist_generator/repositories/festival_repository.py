...
class FestivalRepository(BaseRepository[Festival]):
    ...
    async def get_by_id(
        ...
        if load_relationships:
            query = query.options(
                selectinload(Festival.artists),
                selectinload(Festival.playlists)
            )
    ...
    async def get_upcoming_festivals(
        ...
        result = await self.db.execute(
            select(Festival)
            .where(
                func.array_length(Festival.dates, 1) > 0
            )
        ...
        upcoming = [
            ...
            if any(date >= from_date for date in f.dates)
        ]
