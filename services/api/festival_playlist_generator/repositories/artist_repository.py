...
class ArtistRepository:
    ...
    async def get_by_id(
        ...
        if load_relationships:
            query = query.options(
                selectinload(Artist.festivals),
                selectinload(Artist.setlists)
            )
    ...
    async def search_paginated(
        ...
        if search:
            ...
            base_query = base_query.where(
                or_(
                    ...
                    func.lower(func.array_to_string(Artist.genres, ',')).like(search_term)
                )
            )
        ...
        if filter_with_festivals:
            base_query = base_query.where(
                Artist.id.in_(
                    select(festival_artists.c.artist_id).distinct()
                )
            )
    ...
    async def bulk_delete(self, artist_ids: List[UUID]) -> int:
        ...
        """
        result = await self.db.execute(
            delete(Artist).where(Artist.id.in_(artist_ids))
        )
