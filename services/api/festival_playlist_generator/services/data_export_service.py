...
class DataExportService:
    ...
    async def export_user_data(self, db: AsyncSession, user_id: UUID, 
                              format: str = "json", requester_ip: str = None) -> Dict[str, Any]:
        """Export all user data in the specified format."""
        try:
            # Log the data export request
            await self._log_audit_event(
                db, user_id, "DATA_EXPORT_REQUESTED", 
            ...
            await self._log_audit_event(
                db, user_id, "DATA_EXPORT_COMPLETED",
                details={"format": format, "record_count": len(playlists_data) + len(preferences_data)}
            )
    ...
    async def delete_user_account(self, db: AsyncSession, user_id: UUID, 
                                 requester_ip: str = None) -> Dict[str, Any]:
        """Completely delete user account and all associated data."""
        try:
            # Log the deletion request
            await self._log_audit_event(
                db, user_id, "ACCOUNT_DELETION_REQUESTED",
            ...
            deletion_summary["deleted_records"]["song_preferences"] = preferences_result.rowcount
            ...
            await self._log_audit_event(
                db, user_id, "ACCOUNT_DELETION_COMPLETED",
    ...
    async def _get_user_data(self, db: AsyncSession, user_id: UUID) -> Optional[Dict[str, Any]]:
    ...
    async def _get_user_playlists(self, db: AsyncSession, user_id: UUID) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ...
                "festival_id": str(playlist.festival_id) if playlist.festival_id else None,
    ...
    async def _get_user_song_preferences(self, db: AsyncSession, user_id: UUID) -> List[Dict[str, Any]]:
        """Get user's song preferences."""
        result = await db.execute(select(UserSongPreference).where(UserSongPreference.user_id == user_id))
    ...
    async def _get_user_audit_logs(self, db: AsyncSession, user_id: UUID) -> List[Dict[str, Any]]:
    ...
    async def _log_audit_event(self, db: AsyncSession, user_id: Optional[UUID], 
    ...
    def _convert_to_csv_format(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if data.get("user_profile"):
            ...
            profile_writer = csv.DictWriter(profile_output, fieldnames=data["user_profile"].keys())
        ...
        if data.get("playlists") and len(data["playlists"]) > 0:
            playlists_writer = csv.DictWriter(playlists_output, fieldnames=data["playlists"][0].keys())
            playlists_writer.writeheader()
            playlists_writer.writerows(data["playlists"])
        else:
            # Create empty CSV with headers
            playlists_writer = csv.DictWriter(playlists_output, fieldnames=["id", "name", "description", "festival_id", "artist_id", "platform", "external_id", "created_at", "updated_at"])
            playlists_writer.writeheader()
        ...
        if data.get("song_preferences") and len(data["song_preferences"]) > 0:
            prefs_writer = csv.DictWriter(prefs_output, fieldnames=data["song_preferences"][0].keys())
            prefs_writer.writeheader()
            prefs_writer.writerows(data["song_preferences"])
        else:
            # Create empty CSV with headers
            prefs_writer = csv.DictWriter(prefs_output, fieldnames=["id", "song_id", "is_known", "created_at"])
            prefs_writer.writeheader()
        ...
        if data.get("audit_logs") and len(data["audit_logs"]) > 0:
            audit_writer = csv.DictWriter(audit_output, fieldnames=data["audit_logs"][0].keys())
            audit_writer.writeheader()
            audit_writer.writerows(data["audit_logs"])
        else:
            # Create empty CSV with headers
            audit_writer = csv.DictWriter(audit_output, fieldnames=["id", "action", "resource_type", "resource_id", "ip_address", "user_agent", "details", "created_at"])
            audit_writer.writeheader()
