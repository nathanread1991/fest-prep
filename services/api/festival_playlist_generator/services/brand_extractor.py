...
from festival_playlist_generator.services.color_analyzer import ColorAnalyzer, ColorScheme
...
class BrandExtractor:
    ...
    def __init__(self):
        ...
        if settings.OPENAI_API_KEY:
            ...
            logger.warning("OpenAI API key not configured. AI-powered logo identification will be disabled.")
    
    def extract_all_images(self, html_content: str, base_url: str = "") -> List[ImageInfo]:
        """
    ...
    def _calculate_position_score(self, img_tag, context: str) -> float:
        ...
        if any(keyword in context_lower for keyword in ['header', 'banner', 'logo', 'brand', 'nav']):
            score += 0.3
...
If none of the images appear to be a festival logo, set index to -1 and confidence to 0.0."""
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert at analyzing website structure and identifying brand logos."},
            ...
            logger.info(f"AI logo identification: index={index}, confidence={confidence}, reasoning={reasoning}")
    ...
    async def extract_festival_branding(
        ...
        try:
            ...
            if self.ai_available and logo_candidates:
                logo_url, confidence = await self.extract_logo_with_ai(festival_name, logo_candidates)
                logger.info(f"AI logo identification: url={logo_url}, confidence={confidence}")
            ...
            logger.info(f"Extracted branding: logo={bool(logo_url)}, colors={bool(color_scheme.primary)}")
    ...
    def _fallback_logo_extraction(self, logo_candidates: List[ImageInfo]) -> Optional[str]:
        """
        ...
        for i, (img, score) in enumerate(scored_candidates[:3]):
            logger.info(f"  {i+1}. Score={score:.2f}, URL={img.url}, Alt={img.alt_text}")
