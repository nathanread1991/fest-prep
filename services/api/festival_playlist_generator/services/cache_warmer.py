...
class CacheWarmer:
    ...
    async def warm_cache(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            batch_count = (len(image_urls) + self.max_concurrent - 1) // self.max_concurrent
            for i in range(0, len(image_urls), self.max_concurrent):
                ...
                logger.info(f"Processing batch {batch_num}/{batch_count} ({len(batch)} images)...")
        ...
        for artist in artists:
            if artist.spotify_image_url:
                ...
                if proxy_url:
                    ...
                    internal_url = proxy_url.replace('http://localhost/images/proxy', 
    ...
    async def warm_specific_images(
        self, 
        image_urls: List[str]
    ) -> Dict[str, Any]:
        """
