...
class FestivalService:
    ...
    async def get_upcoming_festivals(
        self, limit: int = 10, load_relationships: bool = False
    ) -> List[Festival]:
        """
    ...
    async def create_festival(
        self, festival: Festival, artist_ids: Optional[List[UUID]] = None
    ) -> Festival:
        """
    ...
    async def add_artist_to_festival(
        self, festival_id: UUID, artist_id: UUID
    ) -> Festival:
        """
        ...
        """
        # Get festival
        festival = await self.festival_repo.get_by_id(
