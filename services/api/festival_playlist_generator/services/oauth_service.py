...
class OAuthProvider:
    ...
    def __init__(self, name: str, client_id: str, client_secret: str, 
                 auth_url: str, token_url: str, user_info_url: str, scopes: List[str]):
        self.name = name
...
class OAuthService:
    ...
    def _initialize_providers(self) -> Dict[str, OAuthProvider]:
        ...
        if (settings.APPLE_CLIENT_ID and settings.APPLE_CLIENT_SECRET and 
            settings.APPLE_PRIVATE_KEY and settings.APPLE_KEY_ID and settings.APPLE_TEAM_ID):
    ...
    async def handle_oauth_callback(self, db: AsyncSession, code: str, state: str, 
        ...
        if is_new_user:
            ...
            logger.info(f"New user OAuth authentication, redirecting to consent: {user.email}")
            return {
                "user": user,
                "is_new_user": True,
    ...
    async def _get_user_info(self, provider: OAuthProvider, access_token: str) -> Dict[str, Any]:
        ...
        if provider.name == "apple":
            ...
            return {"email": "apple_user@example.com", "name": "Apple User"}  # Placeholder
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error(f"User info request failed for {provider.name}: {response.text}")
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if provider.name == "google" or provider.name == "youtube":
                ...
                return {
                    ...
                    "picture": user_data.get("images", [{}])[0].get("url") if user_data.get("images") else None,
                    "provider_id": user_data.get("id")
                }
    ...
    async def _create_or_get_user(self, db: AsyncSession, provider_name: str, 
                                 user_info: Dict[str, Any]) -> tuple[UserSchema, bool]:
        ...
        result = await db.execute(
            select(User).where(
                (User.email == email) | 
                ((User.oauth_provider == provider_name) & (User.oauth_provider_id == provider_id))
            )
        )
    ...
    async def _create_session(self, user_id: UUID) -> str:
        ...
        session_data = {
            ...
            "expires_at": (datetime.utcnow() + timedelta(hours=self.SESSION_EXPIRE_HOURS)).isoformat()
        }
    ...
    async def get_current_user(self, db: AsyncSession, session_id: str) -> Optional[UserSchema]:
