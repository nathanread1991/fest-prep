    ...
    def _initialize_client(self) -> None:
        ...
        if settings.OPENAI_API_KEY:
            try:
                ...
                logger.warning(
                    "openai package not installed, semantic matching disabled"
                )
    ...
    async def semantic_similarity(self, s1: str, s2: str) -> float:
        ...
        try:
            emb1, emb2 = await asyncio.gather(
                self.get_embedding(s1), self.get_embedding(s2)
            )
...
class AdvancedFuzzyMatcher:
    ...
    async def _calculate_comprehensive_score(
        ...
        if query_lower in name_lower or name_lower in query_lower:
            ratio = min(len(query), len(artist_name)) / max(
                len(query), len(artist_name)
            )
        ...
        if query_phonetic[0] and name_phonetic[0]:
            if query_phonetic[0] == name_phonetic[0]:
                ...
                query_phonetic[1]
                and name_phonetic[1]
                and query_phonetic[1] == name_phonetic[1]
            ):
                scores["phonetic_secondary"] = 75
            elif (
                query_phonetic[0] == name_phonetic[1]
                or query_phonetic[1] == name_phonetic[0]
            ):
        ...
        if query_embedding and self.semantic_matcher.client:
            try:
                artist_embedding = await self.semantic_matcher.get_embedding(
                    artist_name
                )
                if artist_embedding:
...
async def advanced_fuzzy_search(
    ...
    return await matcher.search(
        query, limit=limit, min_score=min_score, use_semantic=use_semantic
    )
