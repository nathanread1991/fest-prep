...
class RecommendationEngine:
    ...
    async def analyze_user_preferences(self, user_id: str) -> UserProfile:
        ...
        for pref in song_preferences:
            ...
            if pref.is_known:
                ...
                if song:
                    # Get artist details
                    artist = self.db.query(Artist).filter(Artist.name == song.artist).first()
                    if artist:
        ...
        discovery_rate = (
            ...
            if total_songs_count > 0 else 1.0
        )
    ...
    async def recommend_festivals(self, user_id: str, limit: int = 10) -> List[FestivalRecommendation]:
        """
        ...
        festivals = (
            ...
            .filter(Festival.dates.any(lambda d: d > datetime.utcnow() and d < cutoff_date))
            .all()
        )
        ...
        for festival in festivals:
            ...
            if similarity_score > 0.1:  # Only recommend festivals with some similarity
                matching_artists, recommended_artists = await self._analyze_festival_artists(
                    festival, user_profile
                )
                
                recommendations.append(FestivalRecommendation(
    ...
    async def recommend_artists(
        ...
        festival_artists = (
            ...
            .filter(Artist.name.in_(festival.artists))
            .all()
        )
    ...
    async def _analyze_festival_artists(
        ...
        for artist in festival_artists:
            if str(artist.id) in user_profile.preferred_artists:
                ...
                similarity = await self._calculate_artist_similarity(artist, user_profile)
