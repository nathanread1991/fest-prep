...
class FestivalEnrichmentService:
    ...
    async def fetch_from_clashfinder(self, clashfinder_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _parse_clashfinder_data(self, data: Dict[str, Any], clashfinder_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            enriched_data["name"] = (
                data.get('name') or 
                data.get('title') or 
                data.get('festival_name') or
                data.get('event_name') or
                clashfinder_id.replace('-', ' ').replace('_', ' ').title()
            )
            ...
            logger.info(f"Successfully parsed Clashfinder data: {enriched_data['name']} with {len(artists)} artists")
                        ...
                        for stage_name, stage_data in stages_data.items():
                            if isinstance(stage_data, list):
                                ...
                                if 'artists' in stage_data:
                                    extracted = self._parse_lineup_data(stage_data['artists'])
                                    artists.extend(extracted)
                                elif 'lineup' in stage_data:
                                    extracted = self._parse_lineup_data(stage_data['lineup'])
                                    artists.extend(extracted)
