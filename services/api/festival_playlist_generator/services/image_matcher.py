...
class ImageMatcher:
    ...
    def _find_lineup_section(self, soup: BeautifulSoup) -> Optional[Any]:
        ...
        for keyword in lineup_keywords:
            section = soup.find(
                ["section", "div"],
                attrs={"data-section": re.compile(keyword, re.IGNORECASE)},
            )
            if section:
                return section
    ...
    async def match_images_to_artists(
        self, images: List[ImageInfo], artist_names: List[str]
...
Only include matches where you have reasonable confidence (>0.3). If you can't match an artist, don't include them in the response."""
    ...
    def _fallback_image_matching(
        ...
        for artist_name in artist_names:
            ...
            artist_normalized = (
                artist_name.lower().replace(" ", "").replace("-", "").replace("_", "")
            )
            ...
            for img in images:
                ...
                if img.alt_text:
                    ...
                    if artist_normalized in alt_normalized:
                        ...
                        word in alt_normalized for word in artist_name.lower().split()
                    ):
                        score += 0.4

                # Check URL
                url_normalized = (
                    img.url.lower().replace(" ", "").replace("-", "").replace("_", "")
                )
                if artist_normalized in url_normalized:
                    score += 0.6
                elif any(
                    word in url_normalized for word in artist_name.lower().split()
                ):
                    score += 0.3
