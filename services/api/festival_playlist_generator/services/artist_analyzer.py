...
import httpx
from festival_playlist_generator.schemas.setlist import SetlistCreate, Setlist
...
class SetlistFmClient:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if artists:
                    ...
                    exact_matches = [a for a in artists if a.get("name", "").lower() == artist_name.lower()]
                    
                    if exact_matches:
                        # Use sophisticated scoring algorithm to find the best match
                        best_match = await self._score_and_select_artist(exact_matches, artist_name)
                        if best_match:
                            return best_match
                        else:
                            # Fallback to first exact match
                            self.logger.info(f"Using first exact match for '{artist_name}': {exact_matches[0].get('name')}")
                            return exact_matches[0]
                    else:
                        # No exact match, use scoring on all results
                        best_match = await self._score_and_select_artist(artists, artist_name)
                        if best_match:
                            return best_match
                        else:
                            # Final fallback to first result
                            self.logger.info(f"No exact match for '{artist_name}', using first result: {artists[0].get('name')}")
                            return artists[0]
                else:
                ...
                self.logger.error(f"API error searching for artist {artist_name}: {response.status_code}")
                return None
    ...
    async def _score_and_select_artist(self, artists: List[Dict], search_name: str) -> Optional[Dict]:
        ...
        self.logger.info(f"Selected artist '{best_artist.get('name')}' with score {best_score:.2f} for search '{search_name}'")
        ...
        try:
            ...
            if setlist_response.status_code == 200:
                ...
                if valid_setlists > 0:
                    ...
                    score += min(total_songs / 10, 8.0)  # Up to 8 points for song variety
    ...
    async def get_artist_setlists(self, artist_mbid: str, limit: int = 10) -> List[SetlistData]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"Retrieved {len(parsed_setlists)} setlists for artist {artist_mbid}")
                ...
                self.logger.error(f"API error getting setlists for {artist_mbid}: {response.status_code}")
                return []
    ...
    def _parse_setlist_data(self, setlist_data: Dict) -> Optional[SetlistData]:
        ...
        try:
            ...
            # Extract tour info
            tour_name = setlist_data.get("tour", {}).get("name") if setlist_data.get("tour") else None
            
            # Check if it's a festival
            festival_name = None
            if venue_data.get("name") and "festival" in venue_data.get("name", "").lower():
...
class SongNormalizer:
    ...
    def identify_cover_song(self, song_title: str, performing_artist: str) -> Tuple[bool, Optional[str]]:
        """
...
class SongDeduplicator:
    ...
    def deduplicate_songs(self, songs: List[str], artist_name: str) -> List[Dict[str, any]]:
        """
        ...
        for song in songs:
            ...
            is_cover, original_artist = self.normalizer.identify_cover_song(song, artist_name)
        ...
        for song_info in processed_songs:
            ...
            if normalized in seen_normalized:
                ...
                self.logger.debug(f"Skipping duplicate song: {song_info['original_title']}")
                continue
            
            seen_normalized[normalized] = True
            deduplicated.append(song_info)
        
        self.logger.info(f"Deduplicated {len(songs)} songs to {len(deduplicated)} unique songs")
        return deduplicated
...
class SongFrequencyAnalyzer:
    ...
    def analyze_song_frequency(self, setlists: List[SetlistData]) -> Dict[str, Dict[str, any]]:
        """
        ...
        for setlist in setlists:
            processed_songs = set()  # Track songs in this setlist to avoid double-counting
            ...
            for song in setlist.songs:
                ...
                if normalized_title not in song_frequency:
                    is_cover, original_artist = normalizer.identify_cover_song(song, setlist.artist_name)
        ...
        self.logger.info(f"Analyzed frequency for {len(song_frequency)} unique songs across {len(setlists)} setlists")
        return song_frequency
    
    def rank_songs_by_frequency(self, song_frequency: Dict[str, Dict[str, any]]) -> List[Dict[str, any]]:
        """
...
class ArtistAnalyzerService:
    ...
    async def get_artist_setlists(self, artist_name: str, limit: int = 10) -> List[Setlist]:
        """
        ...
        try:
            async with SetlistFmClient(self.setlist_fm_api_key) as client:
                ...
                if not artist_mbid:
                    self.logger.warning(f"No MusicBrainz ID found for artist: {artist_name}")
                    return []
                ...
                self.logger.info(f"Retrieved and stored {len(setlists)} setlists for {artist_name}")
                return setlists
    ...
    async def analyze_song_frequency(self, setlists: List[Setlist]) -> Dict[str, int]:
        ...
        return {data['original_title']: data['frequency'] for data in frequency_data.values()}
    ...
    async def identify_cover_songs(self, songs: List[str], artist: str) -> Dict[str, str]:
        """
    ...
    async def _store_setlist(self, setlist_data: SetlistData) -> Optional[Setlist]:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(select(ArtistModel).where(ArtistModel.name == setlist_data.artist_name))
