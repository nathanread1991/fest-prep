...
from festival_playlist_generator.schemas.user import UserCreate, User as UserSchema
...
class AuthService:
    ...
    async def register_user(self, db: Session, user_data: UserCreate, password: str) -> UserSchema:
        """Register a new user."""
        ...
        await self.cache.set(f"user_password:{db_user.id}", hashed_password, expire=31536000)  # 1 year
        
        logger.info(f"User registered: {db_user.email}")
        return UserSchema.model_validate(db_user)
    
    async def authenticate_user(self, db: Session, email: str, password: str) -> Optional[UserSchema]:
    ...
    async def get_current_user(self, db: Session, session_id: str) -> Optional[UserSchema]:
