...
from festival_playlist_generator.models.user import UserSongPreference, User
...
class UserPreferenceService:
    ...
    async def mark_song_preference(
        ...
        logger.info(f"Song preference updated: user={user_id}, song={song_id}, known={is_known}")
    ...
    async def get_user_song_preferences(
        ...
        if cached_preferences:
            ...
            # Get from database
            query = select(UserSongPreference).where(UserSongPreference.user_id == user_id)
    ...
    async def get_unknown_songs(self, db: Session, user_id: UUID) -> List[UUID]:
        ...
        preferences = await self.get_user_song_preferences(db, user_id, known_only=False)
        return [pref.song_id for pref in preferences]
    
    async def is_song_known(self, db: Session, user_id: UUID, song_id: UUID) -> Optional[bool]:
    ...
    async def bulk_mark_songs(
        ...
        for song_id, is_known in song_preferences.items():
            try:
                preference = await self.mark_song_preference(db, user_id, song_id, is_known)
                results.append(preference)
            except HTTPException as e:
                logger.warning(f"Failed to update preference for song {song_id}: {e.detail}")
                continue
