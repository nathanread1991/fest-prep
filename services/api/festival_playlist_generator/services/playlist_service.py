...
class PlaylistService:
    ...
    async def update_playlist(self, playlist: Playlist) -> Playlist:
        ...
        if playlist.festival_id:
            await self.cache.delete_pattern(f"playlists:festival:{playlist.festival_id}:*")
    ...
    async def sync_to_spotify(
        ...
        playlist = await self.playlist_repo.get_by_id(playlist_id, load_relationships=True)
        if not playlist:
        ...
        try:
            ...
            if playlist.tracks:
                track_uris = [f"spotify:track:{track.spotify_id}" for track in playlist.tracks if track.spotify_id]
                if track_uris:
            ...
            logger.info(f"Synced playlist {playlist_id} to Spotify: {spotify_playlist_id}")
