...
class SetlistEnrichmentService:
    ...
    def __init__(self, api_key: Optional[str] = None) -> None:
        ...
        if not self.api_key:
            self.logger.warning(
                "Setlist.fm API key not configured. Enrichment will be skipped."
            )

    async def enrich_artists(
        self, artist_ids: List[str], db: AsyncSession, limit: int = 10
    ) -> Dict[str, int]:
        """
        ...
        stats = {"enriched": 0, "skipped": 0, "failed": 0, "total": len(artist_ids)}
        ...
        for artist_id in artist_ids:
            try:
                ...
                if result is None:
                    ...
                    stats["enriched"] += 1
                    self.logger.info(
                        f"Enriched artist {artist_id} with {result} setlists"
                    )
                else:
    ...
    async def enrich_artist_by_id(
        self, artist_id: str, db: AsyncSession, limit: int = 10
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            result = await db.execute(
                select(func.count(SetlistModel.id)).where(
