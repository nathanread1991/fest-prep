...
from festival_playlist_generator.services.name_normalization_service import normalize_artist_name
...
class SmartArtistSearch:
    ...
    async def search(
        ...
        query: str, 
        limit: int = 10,
        include_spotify_fallback: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not formatted_results and include_spotify_fallback:
            logger.info(f"No local matches for '{query}', Spotify fallback could be used")
    ...
    def _calculate_token_score(self, query_tokens: List[str], artist_tokens: List[str]) -> int:
        """
    ...
    def _calculate_contains_score(self, query: str, artist_name: str) -> int:
        ...
        if query_lower in name_lower:
            ...
            position_score = 100 - (name_lower.index(query_lower) / len(name_lower) * 20)
