...
class ArtistEnrichmentService:
    ...
    async def enrich_artists(self, artist_ids: List[str], db: AsyncSession) -> dict:
        ...
        for artist_id in artist_ids:
            try:
                ...
                if artist.spotify_id:
                    logger.info(f"Artist {artist.name} already has Spotify data, skipping")
                    skipped_count += 1
                    continue
                ...
                if spotify_info:
                    ...
                    existing_spotify_result = await db.execute(
                        select(ArtistModel).filter(ArtistModel.spotify_id == spotify_info.id)
                    )
        ...
        try:
            await db.commit()
            logger.info(f"Artist enrichment complete: {enriched_count} enriched, {skipped_count} skipped, {failed_count} failed")
        except Exception as e:
    ...
    async def enrich_artist_by_name(self, artist_name: str, db: AsyncSession) -> Optional[ArtistModel]:
        """
