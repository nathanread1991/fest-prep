...
class StreamingPlatformClient(ABC):
    ...
    @abstractmethod
    async def search_song(self, song: str, artist: str, auth_token: AuthToken) -> List[Track]:
        """Search for a song on the platform."""
        pass
    
    @abstractmethod
    async def create_playlist(self, name: str, description: str, auth_token: AuthToken) -> str:
        """Create a new playlist on the platform."""
        pass
    
    @abstractmethod
    async def add_tracks_to_playlist(self, playlist_id: str, track_ids: List[str], auth_token: AuthToken) -> bool:
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> AuthToken:
        ...
        try:
            ...
            if "authorization_code" in credentials:
                token_info = auth_manager.get_access_token(credentials["authorization_code"])
            elif "refresh_token" in credentials:
                token_info = auth_manager.refresh_access_token(credentials["refresh_token"])
            else:
    ...
    async def search_song(self, song: str, artist: str, auth_token: AuthToken) -> List[Track]:
        ...
        try:
            ...
            for item in results["tracks"]["items"]:
                ...
                is_live = any(keyword in item["name"].lower() for keyword in ["live", "concert", "tour"])
...
class YouTubeMusicClient(StreamingPlatformClient):
    ...
    async def search_song(self, song: str, artist: str, auth_token: AuthToken) -> List[Track]:
        ...
        try:
            ...
            for item in results:
                # Determine if it's a live version
                is_live = any(keyword in item["title"].lower() for keyword in ["live", "concert", "tour"])
                ...
                track = Track(
                    ...
                    duration_ms=item.get("duration_seconds", 0) * 1000 if item.get("duration_seconds") else None,
                    is_live=is_live,
                    platform=StreamingPlatform.YOUTUBE_MUSIC
                )
                tracks.append(track)
...
class StreamingIntegrationService:
    ...
    async def authenticate_user(self, platform: StreamingPlatform, credentials: Dict[str, Any]) -> AuthToken:
    ...
    async def search_song(self, song: str, artist: str, platform: StreamingPlatform, auth_token: AuthToken) -> List[Track]:
    ...
    async def find_cover_song_fallback(self, song: str, original_artist: str, cover_artist: str, 
                                     platform: StreamingPlatform, auth_token: AuthToken) -> Optional[Track]:
        """Find original version when cover is not available."""
        # First try to find the cover version
        try:
            cover_tracks = await self.search_song(song, cover_artist, platform, auth_token)
            if cover_tracks:
                prioritized = await self.prioritize_studio_versions(cover_tracks)
                if prioritized:
                    return prioritized[0]
        except Exception as e:
            logger.warning(f"Cover song search failed for {song} by {cover_artist}: {e}")
        
        # If cover not found or search failed, try original artist
        try:
            original_tracks = await self.search_song(song, original_artist, platform, auth_token)
            if original_tracks:
                prioritized = await self.prioritize_studio_versions(original_tracks)
                if prioritized:
                    return prioritized[0]
        except Exception as e:
            logger.warning(f"Original song search failed for {song} by {original_artist}: {e}")
        
        return None
    
    async def create_playlist(self, name: str, description: str, songs: List[Song], 
                            platform: StreamingPlatform, auth_token: AuthToken) -> Tuple[str, List[str]]:
        ...
        found_tracks = []
        for song in songs:
            tracks = await self.search_song(song.title, song.artist, platform, auth_token)
            if tracks:
        ...
        if found_tracks:
            success = await client.add_tracks_to_playlist(playlist_id, found_tracks, auth_token)
            if not success:
                logger.error(f"Failed to add some tracks to playlist {playlist_id}")
        
        return playlist_id, found_tracks
    
    def generate_playlist_name(self, festival_name: Optional[str] = None, 
