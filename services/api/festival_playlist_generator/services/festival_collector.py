...
from festival_playlist_generator.schemas.festival import FestivalCreate, Festival
...
class ClashfinderAPIClient:
    ...
    def _generate_public_key(self, auth_param: str = "", auth_valid_until: str = "") -> str:
    ...
    def _build_auth_params(self, additional_params: Dict[str, str] = None, 
                          auth_param: str = "", auth_valid_until: str = "") -> Dict[str, str]:
    ...
    async def fetch_clashfinder_data(self, clashfinder_id: str, format_type: str = "json") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error("Clashfinder API authentication failed - check username and private key")
                return None
            elif response.status_code == 403:
                self.logger.error("Clashfinder API access forbidden - check permissions")
                return None
                ...
                self.logger.error(f"Clashfinder API error: {response.status_code} - {response.text}")
                return None
                
        except httpx.TimeoutException:
            self.logger.error(f"Timeout fetching Clashfinder data for: {clashfinder_id}")
            return None
    ...
    async def search_clashfinders(self, search_term: str = "") -> List[Dict[str, Any]]:
        ...
        self.logger.warning("Clashfinder search functionality not yet implemented - API may not support search")
        return []
...
class ClashfinderSource(FestivalDataSource):
    ...
    def __init__(self, username: str, private_key: str, source_name: str = "clashfinder"):
        ...
        self.known_festival_ids = [
            ...
            "coachella2024", "coachella2025",
            "glastonbury2024", "glastonbury2025", 
            "lollapalooza2024", "lollapalooza2025",
            "bonnaroo2024", "bonnaroo2025",
            "outsidelands2024", "outsidelands2025",
            "aclfest2024", "aclfest2025",
            "sxsw2024", "sxsw2025",
            "edc2024", "edc2025",
            "ultra2024", "ultra2025"
        ]
            ...
            async with ClashfinderAPIClient(self.username, self.private_key) as client:
                ...
                for festival_id in self.known_festival_ids:
                    try:
                        ...
                        if festival_data:
                            parsed_festival = await self._parse_clashfinder_data(festival_data, client)
                            if parsed_festival:
                                festivals.append(parsed_festival)
                    except Exception as e:
                        self.logger.error(f"Error processing festival ID '{festival_id}': {e}")
                        continue
    ...
    async def _parse_clashfinder_data(self, data: Dict[str, Any], client: ClashfinderAPIClient) -> Optional[RawFestivalData]:
        ...
        try:
            ...
            location = (data.get('location') or data.get('venue') or 
                       data.get('city') or data.get('place') or "Unknown Location")
                ...
                if isinstance(stages_data, dict):
                    for stage_name, stage_data in stages_data.items():
                        if isinstance(stage_data, list):
                            stage_artists = self._extract_artists_from_lineup(stage_data, client)
                            artists.extend(stage_artists)
                        elif isinstance(stage_data, dict) and 'artists' in stage_data:
                            stage_artists = self._extract_artists_from_lineup(stage_data['artists'], client)
                            artists.extend(stage_artists)
            ...
            # Extract ticket URL
            ticket_url = (data.get('ticket_url') or data.get('tickets') or 
    ...
    def _extract_artists_from_lineup(self, lineup_data: Any, client: ClashfinderAPIClient) -> List[str]:
                ...
                if isinstance(item, str):
                    ...
                    for field in name_fields:
                        if field in item:
                            ...
                            if artist_name:
                                normalized_artist = client.normalize_artist_name_for_setlistfm(str(artist_name))
                                if normalized_artist:
                                    artists.append(normalized_artist)
                                break
                ...
                if response.status_code == 200:
                    ...
                    for event in events:
                        try:
                            ...
                            if not any(keyword in name.lower() for keyword in ['festival', 'fest', 'music']):
                                continue
                            ...
                            artists = []
                            artist_elems = event.find_all('a', href=re.compile(r'/artists/'))
                            ...
                            if name and location and date:
                                festivals.append(RawFestivalData(
                                    source=self.source_name,
                                    name=name,
                                    dates=[date],
                                    location=location,
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    events = soup.find_all('div', class_='eventCard') or soup.find_all('article', class_='event')
                    
                    for event in events:
                        try:
                            # Extract festival name
                            name_elem = event.find('h2') or event.find('h3') or event.find('a', class_='event-name')
                            if not name_elem:
                                continue
                            name = name_elem.get_text(strip=True)
                            ...
                            # Extract location
                            location_elem = event.find('span', class_='venue-name') or event.find('div', class_='location')
                            location = location_elem.get_text(strip=True) if location_elem else ""
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    events = soup.find_all('div', class_='festival-card') or soup.find_all('article', class_='festival')
                    
                    for event in events:
                        try:
                            # Extract festival name
                            name_elem = event.find('h2') or event.find('h3') or event.find('a', class_='festival-name')
                            if not name_elem:
                                continue
                            name = name_elem.get_text(strip=True)
                            
                            # Extract location
                            location_elem = event.find('span', class_='location') or event.find('div', class_='venue')
                            location = location_elem.get_text(strip=True) if location_elem else ""
                            ...
                            # Extract artists from lineup
                            artists = []
                            lineup_elem = event.find('div', class_='lineup') or event.find('ul', class_='artists')
                            if lineup_elem:
                                artist_elems = lineup_elem.find_all('a') or lineup_elem.find_all('li')
                                for artist_elem in artist_elems:
                                    artist_name = artist_elem.get_text(strip=True)
                ...
                if response.status_code == 200:
                    ...
                    for event in potential_events:
                        try:
                            ...
                            if not any(keyword in text.lower() for keyword in ['festival', 'fest', 'music']):
                                continue
                            
                            # Try to extract name (usually in headings or strong tags)
                            name_elem = event.find(['h1', 'h2', 'h3', 'h4', 'strong', 'b'])
                            if not name_elem:
                                continue
                            name = name_elem.get_text(strip=True)
...
class APISource(FestivalDataSource):
    ...
    def __init__(self, api_url: str, api_key: Optional[str] = None, source_name: str = "api"):
        super().__init__(source_name)
                    ...
                    for event in events:
                        try:
                            ...
                            if life_span.get('begin'):
                                try:
                                    begin_date = datetime.fromisoformat(life_span['begin'])
                ...
                if response.status_code == 200:
                    ...
                    for event in events:
                        try:
                            ...
                            if not any(keyword in title.lower() for keyword in ['festival', 'fest']):
                                continue
                            ...
                            if start_date:
                                try:
                                    date = datetime.strptime(start_date, '%a, %d %b %Y %H:%M:%S')
                                except:
                                    continue
                            else:
                                continue
                    ...
                    if search_response.status_code == 200:
                        ...
                        for playlist in playlists:
                            try:
                                ...
                                if any(keyword in name.lower() for keyword in ['festival', 'fest']):
                                    ...
                                    date = datetime.now()  # Would need to parse from name/description
                                ...
                                self.logger.debug(f"Error parsing Spotify playlist: {e}")
                                continue
        except Exception as e:
                ...
                if response.status_code == 200:
                    ...
                    for event in events:
                        try:
                            ...
                            # Extract name
                            name = event.get('name') or event.get('title') or event.get('event_name')
                            if not name:
                                continue
                            
                            # Extract date
                            date_str = event.get('date') or event.get('start_date') or event.get('event_date')
                            if date_str:
                                try:
                            ...
                            # Extract location
                            location = event.get('location') or event.get('venue') or event.get('city') or "Unknown Location"
                            
                            # Extract artists
                            artists = []
                            artist_data = event.get('artists') or event.get('lineup') or event.get('performers') or []
...
class FestivalParser:
    ...
    def parse_festival_data(self, raw_data: RawFestivalData) -> Optional[FestivalCreate]:
        """Parse raw festival data into a validated FestivalCreate object."""
        try:
            # Validate required fields
            if not self._validate_required_fields(raw_data):
                self.logger.warning(f"Missing required fields in festival data: {raw_data.name}")
                return None
    ...
    def _validate_required_fields(self, raw_data: RawFestivalData) -> bool:
        ...
        return (
            raw_data.name and raw_data.name.strip() and
            raw_data.dates and len(raw_data.dates) > 0 and
            raw_data.location and raw_data.location.strip()
        )
    ...
    def _clean_name(self, name: str) -> str:
        ...
        cleaned = re.sub(r'\s+(Festival|Fest|Music Festival)$', '', cleaned, flags=re.IGNORECASE)
...
class FestivalDeduplicator:
    ...
    def deduplicate_festivals(self, festivals: List[FestivalCreate]) -> List[FestivalCreate]:
        ...
        self.logger.info(f"Deduplicated {len(festivals)} festivals to {len(deduplicated)}")
        return deduplicated
    
    def _group_similar_festivals(self, festivals: List[FestivalCreate]) -> List[List[FestivalCreate]]:
    ...
    def _are_festivals_similar(self, festival1: FestivalCreate, festival2: FestivalCreate) -> bool:
        """Check if two festivals are similar enough to be considered duplicates."""
        # Check name similarity
        name_similarity = self._calculate_name_similarity(festival1.name, festival2.name)
        if name_similarity < 0.8:  # Names must be at least 80% similar
            return False
        
        # Check location similarity
        location_similarity = self._calculate_location_similarity(festival1.location, festival2.location)
    ...
    def _have_overlapping_dates(self, dates1: List[datetime], dates2: List[datetime]) -> bool:
        ...
        return (
            abs((start1 - start2).days) <= tolerance_days or
            abs((end1 - end2).days) <= tolerance_days or
    ...
    def _merge_festival_group(self, festivals: List[FestivalCreate]) -> Optional[FestivalCreate]:
        ...
        best_name = max([f.name for f in festivals if f.name], key=len, default=base_festival.name)
        
        # Choose best location (most detailed)
        best_location = max([f.location for f in festivals if f.location], key=len, default=base_festival.location)
...
class FestivalCollectorService:
    ...
    def _initialize_data_sources(self):
        ...
        if settings.CLASHFINDER_USERNAME and settings.CLASHFINDER_PRIVATE_KEY:
            clashfinder_source = ClashfinderSource(
                settings.CLASHFINDER_USERNAME, 
                settings.CLASHFINDER_PRIVATE_KEY
            )
            self.add_data_source(clashfinder_source)
            self.logger.info("Initialized Clashfinder API as primary data source")
        else:
            self.logger.warning("Clashfinder credentials not found, skipping Clashfinder integration")
        ...
        for url in fallback_sources:
            try:
                web_source = WebScrapingSource(url, f"web_scraping_{url.split('//')[1].split('.')[1]}")
                self.add_data_source(web_source)
            except Exception as e:
                self.logger.debug(f"Could not initialize web scraping source for {url}: {e}")
    ...
    async def collect_daily_festivals(self) -> List[Festival]:
        ...
        for source in self.data_sources:
            if isinstance(source, ClashfinderSource):
                try:
                    self.logger.info("Attempting to collect from Clashfinder API (primary source)")
                    raw_festivals = await source.fetch_festivals()
                    if raw_festivals:
                        all_raw_data.extend(raw_festivals)
                        clashfinder_success = True
                        self.logger.info(f"Successfully collected {len(raw_festivals)} festivals from Clashfinder")
                    else:
        ...
        if not clashfinder_success or len(all_raw_data) < 5:  # Threshold for "insufficient data"
            ...
            for source in self.data_sources:
                if not isinstance(source, ClashfinderSource):
                    try:
                        raw_festivals = await source.fetch_festivals()
                        all_raw_data.extend(raw_festivals)
                        self.logger.info(f"Collected {len(raw_festivals)} festivals from {source.source_name}")
                    except Exception as e:
        ...
        self.logger.info(f"Parsed {len(parsed_festivals)} valid festivals from {len(all_raw_data)} raw entries")
        
        # Deduplicate festivals
        deduplicated_festivals = self.deduplicator.deduplicate_festivals(parsed_festivals)
                ...
                for festival_data in festivals:
                    try:
                        ...
                        if existing:
                            ...
                            updated = await self._update_festival(db, existing, festival_data)
                            if updated:
                        ...
                        self.logger.error(f"Error storing festival {festival_data.name}: {e}")
                        continue
    ...
    async def _find_existing_festival(self, db, festival_data: FestivalCreate) -> Optional[FestivalModel]:
    ...
    async def _create_festival(self, db, festival_data: FestivalCreate) -> Optional[Festival]:
        ...
        try:
            ...
            for artist_name in festival_data.artists:
                ...
                if artist_name in artist_images:
                    ...
                    self.logger.info(f"Set logo for artist {artist_name}: {artist.logo_url}")
    ...
    async def _update_festival(self, db, existing: FestivalModel, festival_data: FestivalCreate) -> Optional[Festival]:
        ...
        try:
            ...
            for artist_name in festival_data.artists:
                ...
                if artist_name in artist_images:
                    ...
                    artist.logo_url = image_data.get('logo_url')
                    artist.logo_source = image_data.get('logo_source', 'festival')
                    self.logger.info(f"Updated logo for artist {artist_name}: {artist.logo_url}")
