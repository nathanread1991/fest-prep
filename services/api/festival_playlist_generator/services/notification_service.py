...
class NotificationService:
    ...
    def __init__(self, db: Session):
        ...
        self.from_email = getattr(settings, 'FROM_EMAIL', 'noreply@festivalplaylist.com')
    ...
    async def send_festival_announcement_notification(
        ...
        for user in users:
            ...
            if await self._should_notify_user(
                user.id, 
                NotificationType.FESTIVAL_ANNOUNCEMENT
            ):
    ...
    async def send_artist_lineup_notification(
        ...
        for user in users:
            if await self._should_notify_user(
                user.id, 
                NotificationType.ARTIST_LINEUP_UPDATE
            ):
                notification = Notification(
    ...
    async def _send_push_notification(self, notification: Notification, user: User):
        ...
        print(f"Push notification for user {user.id}: {json.dumps(push_payload, indent=2)}")
    ...
    async def _should_notify_user(
        ...
        if not (type_prefs.get("email_enabled", True) or type_prefs.get("push_enabled", True)):
            return False
    ...
    async def _get_interested_users_for_festival(self, festival: Festival) -> List[User]:
