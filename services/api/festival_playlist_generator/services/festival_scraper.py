...
class FestivalScraper:
    ...
    async def search_and_scrape_festival(self, festival_name: str, year: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        festival_data = await self._ai_extract_festival_data(official_url, festival_name, year)
        
        return festival_data
    
    async def scrape_url_with_ai(self, url: str, festival_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _ai_extract_festival_data(self, url: str, festival_name: str, year: Optional[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with httpx.AsyncClient(headers=self.headers, timeout=30.0, follow_redirects=True) as client:
                ...
                if len(html_content) > max_chars:
                    ...
                    content_candidates = [
                        soup.find('div', class_=lambda x: x and ('lineup' in str(x).lower() or 'artist' in str(x).lower() or 'posts-container' in str(x).lower())),
                        soup.find('section', class_=lambda x: x and ('lineup' in str(x).lower() or 'artist' in str(x).lower())),
                        soup.find('main'),
                        soup.find('article'),
                        soup.find('body')
                    ]
                ...
                logger.info(f"Extracted {len(html_content)} characters of HTML from {url}")
                
                # Use AI to extract festival metadata (dates, location, etc.)
                # But use our directly extracted artists if we found them
                if self.anthropic_api_key:
                    ai_data = await self._extract_with_anthropic(html_content, festival_name, year, url)
                elif self.openai_api_key:
                    ai_data = await self._extract_with_openai(html_content, festival_name, year, url)
                else:
                    logger.error("No AI API key configured (OPENAI_API_KEY or ANTHROPIC_API_KEY)")
                    ai_data = None
                ...
                if ai_data and artists:
                    ...
                    if len(artists) > len(ai_data.get('artists', [])):
                        logger.info(f"Using directly extracted artists ({len(artists)}) instead of AI extracted ({len(ai_data.get('artists', []))})")
                ...
                if ai_data and self.brand_extractor:
                    try:
                        logger.info("Extracting visual branding from festival website...")
                        ...
                        logger.info(f"Branding extracted: logo={bool(branding.logo_url)}, colors={bool(branding.primary_color)}")
                    except Exception as e:
                        logger.warning(f"Branding extraction failed (non-blocking): {e}")
                ...
                if ai_data and self.image_matcher and ai_data.get('artists'):
                    try:
                        ...
                        lineup_images = self.image_matcher.extract_lineup_images(raw_html, url)
                        logger.info(f"Found {len(lineup_images)} lineup images")
                        
                        # Prioritize logo-style images
                        prioritized_images = self.image_matcher.prioritize_logos(lineup_images)
                        
                        # Match images to artists using AI
                        artist_matches = await self.image_matcher.match_images_to_artists(
                            prioritized_images,
                            ai_data['artists']
                        )
                        ...
                        logger.warning(f"Artist image matching failed (non-blocking): {e}")
    ...
    def _extract_artists_from_html(self, soup: BeautifulSoup) -> List[str]:
        ...
        patterns = [
            ...
            ('h3', {'class': lambda x: x and any(keyword in str(x).lower() for keyword in ['title', 'artist', 'performer'])}),
            # Pattern 2: h2 with class containing "title" or "artist"
            ('h2', {'class': lambda x: x and any(keyword in str(x).lower() for keyword in ['title', 'artist', 'performer'])}),
            # Pattern 3: div with class containing "artist-name"
            ('div', {'class': lambda x: x and 'artist' in str(x).lower() and 'name' in str(x).lower()}),
            # Pattern 4: a tags with href containing "/artist/" or "/band/"
            ('a', {'href': lambda x: x and ('/artist/' in str(x).lower() or '/band/' in str(x).lower())}),
        ]
        ...
        for tag, attrs in patterns:
            ...
            for elem in elements:
                ...
                if text and len(text) > 1 and len(text) < 100:  # Reasonable artist name length
                    # Skip common non-artist text
                    skip_words = ['contact', 'faq', 'ticket', 'venue', 'info', 'sign up', 'news', 'home', 'about']
            ...
            if artists:
                logger.info(f"Found {len(artists)} artists using pattern: {tag} {attrs}")
                break
        
        return artists
    
    async def _extract_with_anthropic(self, html_content: str, festival_name: str, year: Optional[str], url: str) -> Optional[Dict[str, Any]]:
...
If you cannot find certain information, use empty strings or empty arrays. Do not include any explanation, only the JSON."""
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                logger.info(f"AI extracted {len(festival_data.get('artists', []))} artists")
                return festival_data
    ...
    async def _extract_with_openai(self, html_content: str, festival_name: str, year: Optional[str], url: str) -> Optional[Dict[str, Any]]:
...
If you cannot find certain information, use empty strings or empty arrays. Do not include any explanation, only the JSON."""
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    ...
                    json={
                        "model": "gpt-3.5-turbo",
                        "messages": [
                            {"role": "system", "content": "You are an expert at parsing HTML and extracting structured data. You understand HTML structure and can identify relevant information even in complex layouts. Always respond with valid JSON only."},
                            {"role": "user", "content": prompt}
                        ],
                        "temperature": 0.1
                    }
    ...
    async def _find_festival_website(self, festival_name: str, year: Optional[str] = None) -> Optional[str]:
    ...
    async def _google_search_scrape(self, festival_name: str, year: Optional[str] = None) -> Optional[str]:
        """Scrape Google search results directly."""
        try:
            search_query = f"{festival_name} {year or ''} festival official website lineup"
                ...
                if response.status_code == 200:
                    ...
                    for link in links:
                        ...
                        if '/url?q=' in href:
                            ...
                            skip_domains = ['google.com', 'youtube.com', 'facebook.com', 'twitter.com', 'instagram.com', 'wikipedia.org']
                            if any(domain in actual_url.lower() for domain in skip_domains):
                                continue
                            
                            # Prefer official-looking domains
                            festival_slug = festival_name.lower().replace(' ', '')
                            if festival_slug in actual_url.lower() or 'festival' in actual_url.lower():
                    ...
                    for link in links:
                        ...
                        if '/url?q=' in href:
                            ...
                            skip_domains = ['google.com', 'youtube.com', 'facebook.com', 'twitter.com', 'instagram.com']
                            if not any(domain in actual_url.lower() for domain in skip_domains):
    ...
    async def _duckduckgo_search(self, festival_name: str, year: Optional[str] = None) -> Optional[str]:
                        ...
                        if link:
                            ...
                            if 'uddg=' in link:
                                ...
                                if not actual_url.startswith('http'):
                                    actual_url = 'https:' + actual_url if actual_url.startswith('//') else 'https://' + actual_url
