...
class PlaylistGeneratorService:
    ...
    async def generate_artist_playlist(
        ...
        try:
            ...
            setlists = await self.artist_analyzer.get_artist_setlists(
                artist.name, limit
            )
            ...
            if not song_frequency:
                self.logger.warning(
                    f"No songs found in setlists for artist: {artist.name}"
                )
                db.close()
                return None
            ...
            song_objects = await self._create_or_find_songs(
                ranked_songs, artist.name, db
            )
    ...
    async def generate_festival_playlist(
        self, festival_id: UUID, user_id: UUID, limit: int = 10
    ) -> Optional[Playlist]:
        """
        ...
        """
        self.logger.info(
            f"Generating festival playlist for festival {festival_id}, user {user_id}"
        )
        ...
        try:
            ...
            festival = (
                db.query(FestivalModel).filter(FestivalModel.id == festival_id).first()
            )
            if not festival:
            ...
            for artist in artists:
                try:
                    ...
                    if not setlists:
                        self.logger.debug(
                            f"No setlists found for artist: {artist.name}"
                        )
                        continue

                    # Analyze song frequency for this artist
                    artist_song_frequency = (
                        await self.artist_analyzer.analyze_song_frequency(setlists)
                    )
                    ...
                    for song_title, frequency in artist_song_frequency.items():
                        ...
                        normalized_titles = (
                            await self.artist_analyzer.normalize_song_titles(
                                [song_title]
                            )
                        )
                        normalized_title = (
                        ...
                        if normalized_title in all_song_frequency:
                            ...
                            if (
                                artist.name
                                not in all_song_frequency[normalized_title]["artists"]
                            ):
            ...
            if not all_song_frequency:
                self.logger.warning(
                    f"No songs found for any artists in festival: {festival.name}"
                )
                db.close()
                return None
            ...
            ranked_songs = self.rank_songs_by_frequency(
                {
                    ...
                    for data in all_song_frequency.values()
                }
            )
    ...
    def rank_songs_by_frequency(self, songs: Dict[str, int]) -> List[Tuple[str, int]]:
        ...
        ranked = sorted(
            ...
            key=lambda x: (
                -x[1],
                x[0],
            ),  # Negative frequency for descending, title for tie-breaking
        )

        self.logger.debug(f"Ranked {len(ranked)} songs by frequency")
        return ranked

    async def check_existing_playlist(
        self,
    ...
    async def apply_user_preferences(
        self, playlist: Playlist, user_id: UUID
    ) -> Playlist:
        """
        ...
        self.logger.debug(
            f"Applying user preferences for user {user_id} to playlist {playlist.id}"
        )
        return playlist

    async def _create_or_find_songs(
        self, ranked_songs: List[Tuple[str, int]], artist_name: str, db: Session
    ) -> List[Song]:
        ...
        for song_title, frequency in ranked_songs:
            ...
            normalized_titles = await self.artist_analyzer.normalize_song_titles(
                [song_title]
            )
            normalized_title = (
                normalized_titles[0]
            ...
            if existing_song:
                ...
                existing_song.performance_count = max(
                    existing_song.performance_count, frequency
                )
                song_objects.append(
                    Song(
                        id=existing_song.id,
                ...
                covers = await self.artist_analyzer.identify_cover_songs(
    ...
    async def _create_playlist(
        self, playlist_create: PlaylistCreate, song_objects: List[Song], db: Session
    ) -> Playlist:
