...
class SpotifyArtistInfo:
    ...
    def medium_image_url(self) -> Optional[str]:
        ...
        best_image = min(
            self.images,
            key=lambda img: abs(img.get('width', 0) - target_width)
        )
...
class SpotifyArtistService:
    ...
    def search_artist(self, artist_name: str) -> Optional[SpotifyArtistInfo]:
        ...
        try:
            ...
            results = self._spotify.search(q=f'artist:{artist_name}', type='artist', limit=1)
    ...
    def search_artists_multiple(self, artist_name: str, limit: int = 5) -> List[SpotifyArtistInfo]:
        ...
        try:
            # Search for the artist
            results = self._spotify.search(q=f'artist:{artist_name}', type='artist', limit=limit)
            
            if not results['artists']['items']:
                logger.info(f"No Spotify results found for artist: {artist_name}")
                return []
            
            return [SpotifyArtistInfo(artist_data) for artist_data in results['artists']['items']]
    ...
    def get_artist_top_tracks(self, spotify_id: str, country: str = 'US') -> List[Dict[str, Any]]:
