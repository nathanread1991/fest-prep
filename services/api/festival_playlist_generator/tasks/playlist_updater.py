...
from festival_playlist_generator.services.playlist_generator import (
            ...
            playlists_to_update = (
                ...
                .filter(
                    or_(
                        ...
                        PlaylistModel.updated_at.is_(
                            None
                        ),  # Handle playlists never updated
                    )
                )
                .all()
            )
            ...
            for i, playlist in enumerate(playlists_to_update):
                try:
                    ...
                    if result["status"] == "success":
                        ...
                        skipped_count += 1
                        logger.info(
                            f"Skipped playlist: {playlist.name} - {result['reason']}"
                        )
                    else:
                        failed_count += 1
                        logger.warning(
...
async def update_single_playlist(
    playlist_id: UUID,
    playlist_generator: PlaylistGeneratorService,
    artist_analyzer: ArtistAnalyzerService,
    db: Session,
    ...
    try:
        ...
        if not playlist:
            return {"status": "failed", "error": f"Playlist not found: {playlist_id}"}
        ...
        if playlist.user_id:
            preferences = (
                db.query(UserSongPreference)
                .filter(UserSongPreference.user_id == playlist.user_id)
                .all()
            )
        ...
        if playlist.festival_id:
            ...
            logger.debug(
                f"Updating festival playlist for festival {playlist.festival_id}"
            )
            ...
            if new_playlist and hasattr(new_playlist, "songs"):
                ...
                if new_song_ids:
                    new_songs = (
                        db.query(SongModel).filter(SongModel.id.in_(new_song_ids)).all()
                    )
...
def update_playlists_for_festival(self: Task, festival_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            playlists = (
                db.query(PlaylistModel)
                .filter(PlaylistModel.festival_id == UUID(festival_id))
                .all()
            )
            ...
            logger.info(
                f"Found {len(playlists)} playlists to update for festival {festival_id}"
            )
