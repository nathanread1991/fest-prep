    ...
    async def fetch_festivals(self) -> List[RawFestivalData]:
        ...
        mock_festivals = [
            RawFestivalData(
                ...
                dates=[datetime(2024, 7, 15), datetime(2024, 7, 16), datetime(2024, 7, 17)],
...
def collect_daily_festivals(self):
    ...
    async def run_workflow():
        async with AsyncSessionLocal() as db:
            try:
                ...
                self.update_state(
                    state='PROGRESS',
                    meta={'status': 'Initializing service orchestrator', 'progress': 10}
                )
                ...
                self.update_state(
                    state='PROGRESS',
                    meta={'status': 'Running daily maintenance workflow', 'progress': 30}
                )
                ...
                if result["status"] == "completed":
                    self.update_state(
                        state='SUCCESS',
                        meta={'status': 'Daily maintenance completed', 'progress': 100, 'result': result}
                    )
                else:
                    self.update_state(
                        state='FAILURE',
                        meta={'status': 'Daily maintenance failed', 'error': result.get('error', 'Unknown error')}
                    )
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Retrying festival collection in {retry_delay} seconds (attempt {self.request.retries + 1}/{self.max_retries})")
...
def collect_festivals_from_source(self, source_config: Dict[str, Any]):
    ...
    """
    try:
        logger.info(f"Collecting festivals from source: {source_config.get('source_name', 'unknown')}")
        ...
        logger.info(f"Collected {len(raw_festivals)} festivals from {source.source_name}")
