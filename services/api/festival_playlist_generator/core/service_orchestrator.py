...
class ServiceOrchestrator:
            ...
            for artist in festival.artists:
                try:
                    ...
                    if setlists:
                        ...
                        self.logger.debug(
                            f"Analyzed {len(setlists)} setlists for {artist.name}"
                        )
                except Exception as e:
                    self.logger.warning(f"Failed to analyze {artist.name}: {e}")
                    workflow_result["errors"].append(
                        f"Artist analysis failed for {artist.name}: {str(e)}"
                    )
            ...
            if create_streaming_playlist and platform:
                ...
                try:
                    ...
                    workflow_result["steps_completed"].append(
                        "streaming_playlist_creation"
                    )
                except Exception as e:
                    self.logger.error(f"Failed to create streaming playlist: {e}")
                    workflow_result["errors"].append(
                        f"Streaming playlist creation failed: {str(e)}"
                    )
    ...
    async def daily_maintenance_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            maintenance_result["statistics"]["festivals_collected"] = len(
                festivals_collected
            )
            ...
            total_festivals_result = await self.db.execute(
                select(func.count(FestivalModel.id))
            )
            total_festivals = total_festivals_result.scalar()

            total_artists_result = await self.db.execute(
                select(func.count(ArtistModel.id))
            )
            total_artists = total_artists_result.scalar()

            total_playlists_result = await self.db.execute(
                select(func.count(PlaylistModel.id))
            )
