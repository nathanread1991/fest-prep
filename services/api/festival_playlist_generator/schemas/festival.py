...
class FestivalBase(BaseModel):
    ...
    location: str = Field(..., min_length=1, max_length=255, description="Festival location")
    ...
    primary_color: Optional[str] = Field(None, max_length=7, description="Primary color (hex format)")
    secondary_color: Optional[str] = Field(None, max_length=7, description="Secondary color (hex format)")
    accent_colors: Optional[List[str]] = Field(None, description="Accent colors (hex format)")
    branding_extracted_at: Optional[datetime] = Field(None, description="When branding was extracted")


class FestivalCreate(FestivalBase):
    """Schema for creating a new festival."""
    artists: List[str] = Field(default_factory=list, description="Artist names in lineup")
    artist_images: Optional[dict] = Field(None, description="Artist image data from scraping")
