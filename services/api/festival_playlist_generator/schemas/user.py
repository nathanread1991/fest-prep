...
class UserBase(BaseModel):
    ...
    connected_platforms: List[str] = Field(default_factory=list, description="Connected streaming platforms")
...
class UserCreate(UserBase):
    ...
    oauth_provider: Optional[str] = Field(None, description="OAuth provider (e.g., spotify)")
...
class User(UserBase):
    ...
    id: UUID
    oauth_provider: Optional[str] = Field(None, description="OAuth provider (google, apple, youtube, spotify, x)")
    oauth_provider_id: Optional[str] = Field(None, description="Provider-specific user ID")
    display_name: Optional[str] = Field(None, description="User display name")
    profile_picture_url: Optional[str] = Field(None, description="Profile picture URL")
    marketing_opt_in: bool = Field(False, description="Marketing communications opt-in")
    known_songs: Optional[List[str]] = Field(default=None, description="Songs marked as known")
    festival_history: Optional[List[UUID]] = Field(default=None, description="Festival attendance history")
    created_at: datetime
